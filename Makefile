...
PROJECT_DESCRIPTION := Build2 grammar for tree-sitter
VERSION := 0.6.0
...
install: all
	install -d ${DESTDIR}${DATADIR}/tree-sitter/queries/build2 ${DESTDIR}${INCLUDEDIR}/tree_sitter ${DESTDIR}${PCLIBDIR} ${DESTDIR}${LIBDIR}
...
ifneq ($(wildcard queries/*.scm),)
	install -m644 queries/*.scm ${DESTDIR}${DATADIR}/tree-sitter/queries/build2
endif
...
uninstall:
	...
	${RM} -r ${DESTDIR}${DATADIR}/tree-sitter/queries/build2
